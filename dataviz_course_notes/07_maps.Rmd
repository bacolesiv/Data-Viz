...
```{r 07-maps-9}
#Strictly design elements for your visualiations 
#This seems usfual once you find a theme that works for you. Perhaps the one from Biso? 

theme_map <- function(base_size=9, base_family="") {
    require(grid)
    theme_bw(base_size=base_size, base_family=base_family) %+replace%
        theme(axis.line=element_blank(), #Element blank just turns off the theme but you can put whatever you want there 
              axis.text=element_blank(), #Usful for geography so you can remove unecessary lines from coordinates, etc. 
...
p0 <- ggplot(data = subset(us_states_elec,
                           region %nin% "district of columbia"), #removing DC from the data "%not in %"
...
```{r county2}
county_data %>%
    dplyr::select(id, name, state, pop_dens, pct_black) %>%
    sample_n(5)
```
